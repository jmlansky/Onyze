...
namespace Touch.Repositories.Archivos.Repositories
{
    public class ArchivosRepository: SingleEntityComunRepository<Archivo>, IArchivosRepository
    {
        ...
        public async Task<bool> DeleteAllButOriginal(long id)
        {
        ...
        {
            Sql = "update archivo set eliminado = 1, modificado = @modificado where (id_archivo_original = @id_original or id = @id_original) and eliminado = 0";
            Parameters = new Dictionary<string, object>()
            {
                { "modificado", DateTime.Now},
                { "id_original", id }
